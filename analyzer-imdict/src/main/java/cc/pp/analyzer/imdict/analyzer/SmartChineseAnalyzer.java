...
package cc.pp.analyzer.imdict.analyzer;
...
import cc.pp.analyzer.imdict.core.WordTokenizer;
 ...
 *
 * @see cc.pp.analyzer.imdict.core.AnalyzerProfile
 *
 */
public class SmartChineseAnalyzer extends Analyzer {
  ...
  private final WordSegmenter wordSegment;
  ...
  public SmartChineseAnalyzer(boolean useDefaultStopWords) {
    if (useDefaultStopWords) {
			//      stopWords = loadStopWords(this.getClass().getResourceAsStream("stopwords.txt"));
			stopWords = loadStopWords(Thread.currentThread().getContextClassLoader()
