...
import edu.fudan.nlp.cn.tag.Format.Seq2List;
import edu.fudan.nlp.cn.tag.Format.Seq2String;
...
import edu.fudan.nlp.pipe.seq.String2SequenceNoENG;
import edu.fudan.nlp.pipe.seq.String2SimpleSequence;
...
public class CWSTagger extends AbstractTagger {
	...
	public void setEnFilter(boolean b){
		...
		if(isEnFilter)
			...
			prePipe = new String2SequenceNoENG();
	}
	...
	public void setDictionary(THashSet<String> newset) {
		...
		setDictionary(new Dictionary(Dictionary.format(al), true));
	...
	public String tag(String src) {
		...
		try {
			for (int i = 0; i < sents.length; i++) {
				...
				String s = Seq2String.format(inst, preds,delim);
				tag += s;
				if (i < sents.length - 1)
					tag += delim;
			}
	...
	public ArrayList<String> tag2List(String src) {
		...
		try {
			...
			String[] preds = _tag(inst);
			res  = Seq2List.format(inst, preds);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return res;
